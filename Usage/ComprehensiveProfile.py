...
def main():
    parser = argparse.ArgumentParser(description="Comprehensive Dynamic Profiler")
    parser.add_argument("-m", "--module", action="store_true", help="Run library module as a script")
    parser.add_argument("--output", default="Comprehensive_Profile.xlsx", help="Output Excel filename")
    parser.add_argument("--exclude-libs", action="store_true", help="Exclude calls/functions in site-packages/stdlib")
    ...
    for (p_key, c_key), call_count in analyzer.calls.items():
        ...
        if args.exclude_libs and is_lib(p_file) and is_lib(c_file):
            continue

        p_mod, p_cls, p_func = p_meta.get('module', 'unknown'), p_meta.get('class'), p_meta.get('name', 'unknown')
        p_path = f"{p_mod}.{p_cls}.{p_func}" if p_cls else f"{p_mod}.{p_func}"
        c_mod, c_cls, c_func = c_meta.get('module', 'unknown'), c_meta.get('class'), c_meta.get('name', 'unknown')
        c_path = f"{c_mod}.{c_cls}.{c_func}" if c_cls else f"{c_mod}.{c_func}"
        
        edge_results.append({
            'Caller Path': p_path, 'Caller File': p_file,
            'Callee Path': c_path, 'Callee File': c_file,
    ...
    for key, count in analyzer.executions.items():
        ...
        if args.exclude_libs and is_lib(file_path):
            continue

        mod, cls, func = meta.get('module', 'unknown'), meta.get('class'), meta.get('name', 'unknown')
    ...
    if edge_results or flat_results:
        try:
            with pd.ExcelWriter(args.output) as writer:
                if edge_results:
                    df_edge = pd.DataFrame(edge_results).sort_values(by='Call Count', ascending=False)
                    df_edge.to_excel(writer, sheet_name='Edge Profile', index=False)
                if flat_results:
                    df_flat = pd.DataFrame(flat_results).sort_values(by='Execution Count', ascending=False)
                    df_flat.to_excel(writer, sheet_name='Flat Profile', index=False)
        except Exception as e:
            print(f"Error saving to Excel: {e}")
            if edge_results:
                pd.DataFrame(edge_results).to_csv(args.output.replace('.xlsx', '_edge.csv'), index=False)
            if flat_results:
                pd.DataFrame(flat_results).to_csv(args.output.replace('.xlsx', '_flat.csv'), index=False)
